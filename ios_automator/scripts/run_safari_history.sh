#!/usr/bin/env bash
# Full pipeline: preflight → stack → Safari History (visible visits + screenshots).
set -euo pipefail
...
export ROOT
export RUN_LABEL="run_safari_history"
...
log_run_start "run_safari_history"
preflight
...
python ios_automator/automator.py --skip-wda-install safari-history --http "http://127.0.0.1:${WDA_PORT:-8100}"
exit $?

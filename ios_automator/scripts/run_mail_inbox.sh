#!/usr/bin/env bash
# Full pipeline: preflight → stack → Apple Mail Inbox (visible senders + screenshots).
set -euo pipefail
...
export ROOT
export RUN_LABEL="run_mail_inbox"
...
python ios_automator/automator.py --skip-wda-install mail-inbox --http "http://127.0.0.1:${WDA_PORT:-8100}"
exit $?
